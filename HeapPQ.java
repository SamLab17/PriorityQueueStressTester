 ...
 * @author Mike Scott
 * source: https://piazza.com/class/ks97vr6r13t1rb?cid=1384
 */
public class HeapPQ<E extends Comparable<? super E>> implements PQ<E>{
    ...
    public void enqueue(E newValue) {
        // ensure capacity
        if (size + 1 == con.length) {
            enlargeArray(con.length * 2);
        }
        size++;
        int indexToAdd = size;
        int indexOfParent = indexToAdd / 2;
        while (indexToAdd > 1 
                && newValue.compareTo(con[indexOfParent]) < 0) {
            // newValue is less than the value in the parent node
            // so move the value in the parent node down
            con[indexToAdd] = con[indexOfParent];
            indexToAdd = indexOfParent;
            indexOfParent /= 2;
        }
        // we have found the right spot
        con[indexToAdd] = newValue;
    }
